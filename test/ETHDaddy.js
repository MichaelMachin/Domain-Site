...
describe("ETHDaddy", () => {
  ...
  beforeEach(async () => {
    ...
    //List a domain
    const transaction = await ethDaddy.list('Name', tokens(1))
  })
  ...
  describe("Domain", () => {
    it('Returns domain attributes', async () => {
      ...
      expect(domain.cost).to.equal(tokens(1))
      expect(domain.isOwned).to.equal(false)
    })
  })

  describe("Only owner", () => {
    it('Only owner can call list function', async () => {
      await expect(ethDaddy.connect(owner1).list("Name 2", tokens(7))).to.be.revertedWith("Can only be called by the owner")
    })
  })
